...
class BaseSQL:
    ...
    def connect2database(self, database_name: str):
        if (database_name is None or database_name == '') and ('database' not in self.CONN and self.CONN['database'] is None):
    ...
    def connect(self):
        ...
        if 'database' not in self.CONN.keys() or self.CONN['database'] is None:
        ...
        try:
            ...
            if 'database' in self.CONN.keys() and self.CONN['database'] is not None:
        ...
        if 'database' not in self.CONN.keys() or self.CONN['database'] is None:
            ...
            self.active_database = self.CONN['database']
