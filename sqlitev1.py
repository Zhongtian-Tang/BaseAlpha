...
class BaseSQL:
    ...
    def __init__(self, conn: Optional[dict] = None, read_only: bool = False) -> None:
        self.state = False
    ...
    def list_tables(self):
        ...
        self.execute("select name from sqlite_master where type = 'table' order by name")
    ...
    def list_fields(self, table_name: Optional[str] = None, detail: bool = False) -> Union[List, Dict]:
        ...
        if not detail:
            ...
            # return [tuple[0] for tuple in self.cr.description]
            return [tuple_[1] for tuple_ in self.cr]
        else:
            _data_type: dict[str, str] = {tuple_[0]: tuple_[1] for tuple_ in self.cr}
            return {k: self._cls_sql_map[v.lower()] if (v.lower() in self._cls_sql_map.keys()) else v for k, v in _data_type.items()}

    def _check_db_status(self):
        assert self.state, '未连接至数据库！'
