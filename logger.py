...
LOGGING_CONFIG = {
    ...
    "handlers": {
        ...
        "timefile": {
            ...
            "when": 'd',
            "interval": 1,
            "filename": "log/timelog.log",
            "formatter": "default",
        }
    },
    "loggers": {
        "ba": {
            "handlers": ["console", "file", "timefile"],
            "level": "INFO",
            "propagate": False,
        }
    },
...
def get_file_config(file_name: str):
    temp = FILE_CONFIG.copy()
    temp['handlers']['file'].update({
        "filename": "log/%s_log.txt" % file_name,
    })
    temp['loggers'].update({file_name: _default_file_config})
    return temp
...
def get_module_logger(module_name, level: Optional[int] = None):
    ...
    if module_name.startswith("file."):
        module_name = module_name.replace('file.', '')
        config = get_file_config(module_name)
        set_log_with_config(config)

    elif not module_name.startswith("ba."):
        ...
        module_name = "ba.{}".format(module_name)

    # Get logger.
    module_logger = BaseLogger(module_name)
...
def set_log_with_config(log_config: Dict[Text, Any] = LOGGING_CONFIG):
    """set log with config
